...
import static ProgressTheSpire.ProgressTheSpire.makeImagePath;
    ...
    public static Texture getTexture(final String textureString) {
        if (textures.get(textureString) == null) {
            try {
                ...
                if (!textureString.equals(makeImagePath("ui/missing.png"))) {
                    return getTexture(makeImagePath("ui/missing.png"));
                } else {
                    // If we're already trying to load the missing texture, create a fallback texture
                    Texture fallbackTexture = new Texture(1, 1, Pixmap.Format.RGBA8888);
                    textures.put(textureString, fallbackTexture);
